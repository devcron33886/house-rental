...
class HouseController extends Controller
{
    ...
    public function store(StoreHouseRequest $request)
    {
        ...
        if ($request->input('cover_photo', false)) {
            $house->addMedia(storage_path('tmp/uploads/' . basename($request->input('cover_photo'))))->toMediaCollection('cover_photo');
        }

        foreach ($request->input('photos', []) as $file) {
            $house->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('photos');
        }
